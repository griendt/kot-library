...
@overwrite
@section('card-detail')
    Crafting thrones goes in a linear manner: there is only one way to go from one throne to the next, and that is by maxing the current throne first. Each throne upgrade requires resources in the form of some {!! res('gold') !!} <mark>gold</mark> and some <mark>magic ingredients</mark>. At first, you will mostly need {!! res('leaf') !!}<mark>leaves</mark> and {!! res('dust') !!}<mark>dust</mark>, but later on, the throne upgrade will require mostly {!! res('mush') !!}<mark>mushrooms</mark> and {!! res('eye') !!}<mark>eyeballs</mark>.<hr>
